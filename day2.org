...
def checkGamePossible(game: Game, numCubes):
    gameSetBools = []
    for cubeSet in game.cubeSets:
        gameSetBools.append(checkSetPossible(cubeSet, numCubes))

    for val in gameSetBools:
        if val == False:
            return False

    return True

def checkSetPossible(gameSet, numCubes):
    if (gameSet["red"] > numCubes["red"]) or (gameSet["blue"] > numCubes["blue"]) or (gameSet["green"] > numCubes["green"]):
        return False
...
result = 0
for game in games:
    gameVal = checkGamePossible(game, numCubes)
